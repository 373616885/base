...
堆针对一个JVM进程来说是唯一的，也就是一个进程只有一个JVM，但是进程包含多个线程，他们是共享同一堆空间的。（还有一个唯一的Runtime实例）
...
**<u>Java堆区在JVM启动的时候即被创建，其空间大小也就确定了</u>**。是JVM管理的最大一块内存空间。
...
所有的线程共享Java堆，但还可以划分线程私有的缓冲区（Thread Local Allocation Buffer，TLAB）。
...
通常建议将-Xms和-Xmx两个参数配置相同的值，其目的是**为了能够在ava垃圾回收机制清理完堆区后不需要重新分隔计算堆区的大小，从而提高性能**。（不够不断扩容，闲的时候不断的释放，造成性能影响）
...
AdaptiveSizePolicy(自适应大小策略) ：GC的情况自动计算计算 Eden、From 和 To 区的大小
...
- new的对象先放伊甸园区的TALB里面（TALB很小一块，解决线程竞争问题）
- 不够，就再从Eden里分配。此区有大小限制。
...
当我们进行一次垃圾收集后，红色的将会被回收，而绿色的还会被占用着，存放在S0(Survivor From)区。同时我们给每个对象设置了一个年龄计数器，一次回收后就是1（age =1）。
...
Survivor区被动回收，是Eden区满了的时候，触发YongGC/MinorGC，GC将Eden区和S0去的对象一起回收

YongGC/MinorGC之后 Eden区一定是空的
...
- Minor GC / YoungGC：新生代的GC
- Major GC / OldGC：老年代的GC
...
触发条件：当年轻代空间不足时，就会触发MinorGC

这里的年轻代满指的是Eden代满，Survivor满不会引发GC。（每次Minor GC会清理年轻代的内存）

Survivor区GC，是被动的，在MinorGC的时候顺带的
...
- 优先分配到Eden
  - 开发中比较长的字符串或者数组，会直接存在老年代
- 大对象直接分配到老年代
  - 尽量避免程序中出现过多的大对象
  - 大对象直接进人老年代会触发Minor GC，Minor GC会STW，而且大对象下次就不用了，这样会很费性能
  - 大对象进人老年代，只有触发Major GC或者FullGC，才会回收，触发就STW，费性能，费空间，对于朝生夕死的大对象很不划算
- 长期存活的对象分配到老年代
- 动态对象年龄判断
...
尽管不是所有的对象实例都能够在TLAB中成功分配内存（空间比较小），但JVM确实是将TLAB作为内存分配的首选。

在程序中，开发人员可以通过选项“**-XX:UseTLAB**”设置是否开启TLAB空间。默认开启的

默认情况下，TLAB空间的内存非常小，仅占有整个Eden空间的 **1%** ，当然我们可以通过选项“-XX:TLABWasteTargetPercent”设置TLAB空间所占用Eden空间的百分比大小。
...
Survivor 过小，导致对象在YongGC后，容易不进入Survivor 区，直接进入Old 区

这样Minor GC ，就失去意义了，Survivor 区就是让对象尽可能留在Young 区



Eden区过小，Minor GC频率过高，STW过高，影响用户进程
...
JDK7 之后HandlePromotionFailure参数失效

JDK7 之后只要老年代的连续空间大于新生代对象总大小或者历次晋升的平均大小就会进行Minor GC，否则将进行FullGC
...
在动态编译同步块的时候，JIT编译器可以借助逃逸分析来判断同步块所使用的锁对象是否只能够被一个线程访问而没有被发布到其他线程。如果没有，那么JIT编译器在编译这个同步块的时候就会取消对这部分代码的同步。这样就能大大提高并发性和性能。这个取消同步的过程就叫同步省略，也叫做**锁消除**。
...
栈上分配，同步省略：是代码加载到内存以后，JVM 优化的，javap 编译还是能看到的
...
class对象，class类型信息、class类常量池、即时编译器编译后的代码缓存等
