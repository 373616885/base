...
### 执行引擎的工作流程
 ...
 > **执行引擎工作过程**
  ...
  4. 从外观上来看，所有的Java虚拟机的执行引擎输入、处理、输出都是一致的：输入的是字节码二进制流，处理过程是字节码解析执行、即时编译的等效过程，输出的是执行结果。
...
2. 后面绿色（解释执行）和蓝色（JIT即时编译）才是JVM需要考虑的过程

![image-20200710082141643](images/image-20200710082141643.png)

3. javac编译器（前端编译器--橙色的过程）流程图如下所示：

![image-20200710082433146](images/image-20200710082433146.png)

4. Java字节码的执行是由JVM执行引擎来完成（上面是蓝色色的过程，下面的是绿色的过程），流程图如下所示
...
JIT（Just In Time Compiler）编译器：就是虚拟机将源代码直接编译成和本地机器平台相关的机器语言。执行过程中可以优化代码
...
java 使用解释器 就是  编译型-- 一条一条指令 翻译成 机器码  慢

慢 ：执行引擎每次都一条一条指令翻译成机器码

java 使用JIT编译器 就是  解释型 --  将整个函数 翻译成 机器码  快 

快的原因：

不需要一条一条去翻译，将整个方法 翻译成 机器码

还有代码缓存（在方法区中）

还有热点探测功能优化（代码优化，标量替换，栈上分配，锁消除）
...
解释器可以马上发挥作用，省去编译的时间，服务端一般启动时间是关注重点



-Xcomp 编译模式，没有让JVM启用JIT编译器的全部功能，因为有些功能需要执行一定的执行后分析
...
同时，解释执行在   编译器进行激进优化不成立的时候，作为编译器的后补。
...
在Server模式下是10000次。1万次

超过这个阈值，就会触发JIT编译

阈值参数设置： -XX:CompileThreshold

热点探测：方法执行的次数

热点衰减：

不是一直等待，次数到一定的时候一定会启动JIT
...
JIT编译器 这方面一般不需要调整，1万次系统很容易执行到的，一个方法执行1万次相对于系统的调用九牛一毛
...
![](images\image-23200705205444003.jpg)
...
第二种是编译执行（将方法直接编译成机器码）。现代虚拟机为了提高执行效率，会使用即时编译技术（JIT，Just In Time）将方法编译成机器码后再执行
...
2. JIT编译器：HotSpot VM的C1、C2编译器（client模式，server模式。
...
- 方法内联（多个方法融合在一起）：将引用的函数代码编译到引用点处，这样可以减少栈帧的生成，减少参数传递以及跳转过程
- 去虚拟化：对唯一的实现接口进行内联
...
就算指定 -server JVM 还是C1和C2配合使用的，所以上面的优化策略是一起使用的
...
java 17 HotSpot VM 删除了AOT，因为几乎没有人使用，但需要大量维护工作，如果要使用使用 Graalvm
...
- 还需要继续优化中，最初只支持Linux X64 java base（现在支持 Windows了）
