...
public class LocalVarGC {
     ...
     * 局部变量表索引为1的槽中的变量，还是buffer，没有被替换
     */
    ...
    public void localvarGC5() {
        //调用localvarGC1的时候，里面的System.gc();没有回收buffer
        //外面的System.gc();回收了buffer
        localvarGC1();
        System.gc();
    }
...
3. 大多数情况下，GC会进行各种年龄段的垃圾回收，实在不行了就放大招，来一次独占式的Full GC操作，这时候会回收大量的内存，供应用程序继续使用。（**OOM之前来一次独占式的Full GC，Full GC会先触发一次Minor GC,减轻Full GC 的负担**）
...
这里面隐含着一层意思是，在抛出OutofMemoryError之前，通常垃圾收集器一定会被触发，尽其所能去清理出空间。
...
也称作“存储渗漏”。**严格来说，只有对象不会再被程序用到了，但是GC又不能回收他们的情况，就叫内存泄漏。**
...
>
> 对象不用了，但无法回收（代码不规范没有回收）
>
> static 的属性对象，只用一次就不用了，这个对象就是内存泄漏
...
- static 的属性对象，只用一次就不用了，这个对象被class引用，一直都不能被回收，也是内存泄漏
...
**简单理解：空间换时间，加载和JIT编译的时候，OopMap记录栈里引用的位置，GC Roots的时候直接扫描 OopMap就可以了，不需要一个不漏地从方法区等GC Roots开始查找**

**注意只有在 安全点  才生成 OopMap**
...
- 当线程运行到Safe Region的代码时，首先标识已经进入了Safe Relgion，如果这段时间内发生GC，JVM会忽略----标识为Safe Region状态的线程
...
![img](images\Snipaste_2020-10-08_11-14-83.png)
...
我们希望能描述这样一类对象：当内存空间还足够时，则能保留在内存中；
...
- 强引用（StrongReference）：最传统的“引用”的定义，是指在程序代码之中普遍存在的引用赋值，即类似“object obj=new Object（）”这种引用关系。无论任何情况下，**只要强引用关系还存在，垃圾收集器就永远不会回收掉被引用的对象。就算OOM挂了也不能回收**
- 软引用（SoftReference）：在系统将要发生内存溢出之前，将会把这些对象列入回收范围之中进行第二次回收。如果这次回收后还没有足够的内存，才会抛出内存流出异常。（**缓存的场景**）
- 弱引用（WeakReference）：被弱引用关联的对象只能生存到下一次垃圾收集之前。当垃圾收集器工作时，无论内存空间是否足够，都会回收掉被弱引用关联的对象。（**缓存的场景--如：WeakHashMap**）
...
public class StrongReferenceTest {
    public static void main(String[] args) {
        StringBuffer str = new StringBuffer ("Hello,qinjp");
...
或者 一行当三行使用
SoftReference<User> userSoftRef = new SoftReference<User>(new Object());
```
...
## SoftReference设计对象Cache

案例一：

```java
package com.qin.demo.gc;
...
public class SoftReferenceTest {
    ...
    static SoftReference<Map<String, String>> softReference = null;
...
public class SoftReferenceTest {
    ...
    static SoftReference<Object> softReference = null;

    public static Object getCache() {
        return softReference.get();
    }

    public static void setCache(Object obj) {
...
public class ImageLoader {
    ...
    public void loadImage(final String path, final Callback callback) {
     ...
     //缓存中获取图片，本质将原引用 Bitmap 对象，变成从 SoftReference 对象 
     //变量还是保存的只不过保存的是 SoftReference 对象 ，不保存 Bitmap 对象
     public Bitmap getBitmapByPath(String path) {
        // 从缓存中取软引用的Bitmap对象
        SoftReference<Bitmap> softBitmap = cacheImage.get(path);
        // 判断是否存在软引用
        if (softBitmap == null) {
            return null;
        }
...
tomcat 中有使用其做缓存，非常巧妙：[tomcat/ConcurrentCache.java at trunk · apache/tomcat · GitHub](https://link.zhihu.com/?target=https%3A//github.com/apache/tomcat/blob/trunk/java/org/apache/tomcat/util/collections/ConcurrentCache.java)
        ...
        public void run() {
            while (true) {
                if (phantomQueue != null) {
                    ...
                    try {
                        //Remove方法是个阻塞方法
                        //当垃圾回收器决定对PhantomReferenceTest对象进行回收时，会将其虚引用对象插入ReferenceQueue中
...
NIO中可以使用堆外的内存，也叫直接内存，直接内存的回收就是使用的虚引用通知机制

ByteBuffer byteBuffer = ByteBuffer.allocateDirect(10);

allocateDirect方法会创建一个DirectByteBuffer对象，下面就是构造方法：

![](images\Snipaste_2020-10-08_11-14-84.png)

在构造方法中，可以看到虚引用对象  `Cleaner` （继承了 PhantomReference）引用了 ByteBuffer 本身 （this）

详细查看Cleaner的源码前，先看Deallocator类，这个类就是一个Runnable，在run方法中通过Unsafe类的freeMemory方法释放内存。

![](images\Snipaste_2020-10-08_11-14-85.png)

类Cleaner继承于PhantomReference，其中有一个方法clean，这个方法执行清理工作，如果收到回收的信号，就会执行Deallocator的run方法。在包 jdk.internal.ref 不要弄错了

![](images\Snipaste_2020-10-08_11-14-86.png)

jvm 虚引用机制：当垃圾回收器决定对 ByteBuffer  对象进行回收时，会将其虚引用对象 `Cleaner` 插入ReferenceQueue中

专门线程 ReferenceHandler 处理监听的 ReferenceQueue，执行 clean 方法
...
启动 ReferenceHandler  则在 Reference 的静态代码块中
