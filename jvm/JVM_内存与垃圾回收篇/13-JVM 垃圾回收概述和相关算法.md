...
5% 在方法区 或者 个别JVM方法区没有垃圾收集

方法区 主要存储：类元信息 和 运行时常量池  这两个地方很少变动

变动：

运行时常量池废弃的常量和类被卸载了

类被卸载了对应的--字符串常量（堆中），类型信息（方法区中），运行常量池（方法区中）都要被回收
...
2. 除了释放没用的对象，垃圾回收也可以清除内存里的记录碎片。碎片整理将所占用的堆内存移到堆的一端，**以便JVM将整理出的内存分配给新的对象**。（大对象需要连续的大空间）
