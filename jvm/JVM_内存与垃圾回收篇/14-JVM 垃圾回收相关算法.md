...
3. 优点：
   ...
   实现简单，垃圾对象便于辨识；判定效率高，回收没有延迟性。随时回收，引用计数器的值为0就回收，不需要等内存不够了再去回收
...
```
[PSYoungGen: 13569K->808K(38400K)] 13569K->816K(125952K)
```
...
2. 除了这些固定的GC Roots集合以外，根据用户所选用的垃圾收集器以及当前回收的内存区域不同，还可以有其他对象“临时性”地加入，共同构成完整GC Roots集合。比如：分代收集和局部回收（PartialGC）。
   ...
   - 例如：新生代的对象被老年代引用，现在只收集新生代，这时老年代的也加人GC Roots
...
**有一致性的要求，所以导致GC进行时必须“Stop The World”的一个重要原因**
...
- 不可触及的：对象的finalize（）被调用，并且没有复活，那么就会进入不可触及状态。不可触及的对象不可能被复活，因为 **finalize()只会被调用一次**。（只能喊一次刀下留人）
...
**方式一：命令行使用 jmap**![image-20200712112026317](images/image-20200712112026317.png)
...
![](images\Snipaste_2020-10-08_11-14-65.png)

2、 点击【堆 Dump】

![img](D:\GitHub\base\jvm\vm\images\Snipaste_2020-10-08_11-14-66.png)

3、右键 --> 另存为即可

![](images\image-23200705205444008.jpg)

4、输入命令，继续执行程序

![img](images\Snipaste_2020-10-08_11-14-67.png)

5、我们接着捕获第二张堆内存快照

![](images\image-23200705205444009.jpg)
...
![img](images\Snipaste_2020-10-08_11-14-72.png)

2、

![img](images\Snipaste_2020-10-08_11-14-73.png)

![img](images\Snipaste_2020-10-08_11-14-74.png)

可以发现颜色变绿了，可以动态的看变化

3、右击对象，选择 Show Selection In Heap Walker，单独的查看某个对象

![img](images\Snipaste_2020-10-08_11-14-75.png)

![img](images\Snipaste_2020-10-08_11-14-76.png)

4、选择Incoming References，表示追寻 GC Roots 的源头

点击Show Paths To GC Roots，在弹出界面中选择默认设置即可

![img](images\Snipaste_2020-10-08_11-14-77.png)

![img](images\Snipaste_2020-10-08_11-14-78.png)

![img](images\Snipaste_2020-10-08_11-14-79.png)
...
1、看这个超大对象![img](images\Snipaste_2020-10-08_11-14-80.png)

2、揪出 main() 线程中出问题的代码

![img](images\Snipaste_2020-10-08_11-14-81.png)
...
1. 标记清除算法的效率不算高（两次全遍历，一次标记，一次清除）
 ...
 **特别的优点缺点**
	 ...
	 1. 如果系统垃圾对象很多，复制算法不会很理想，因为如果垃圾多复制算法需要复制的对象数量变多，效率会很低，优点变缺点（GC 没有回收一个对象，很崩溃）
  	 2. 符合年轻代朝生夕死的新生代一次通常可以回收70% - 99% 的内存空间，回收性价比很高，缺点变优点
...
![](images\Snipaste_2020-10-08_11-14-82.png)
...
1. 年轻代（Young Gen）
   ...
   - 这种情况复制算法的回收整理，速度是最快的。复制算法的效率只和当前存活对象大小有关，因此很适用于年轻代的回收。而复制算法内存利用率不高的问题，通过hotspot中的两个survivor的设计得到缓解。Eden : survivor  8 : 1 ：1 浪费的比较少，才 1/10 
2. 老年代（Tenured Gen）
