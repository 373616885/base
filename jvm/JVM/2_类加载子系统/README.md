...
- 类加载器：只负责加载Class文件，至于是否可以执行由 执行引擎决定
- 执行引擎
...
这里不包含用final修饰的static，因为final在编译的时候就会分配了，准备阶段（当前阶段）会显式初始化；
...
矛盾点：要在BootstrapClassLoader加载的类里，调用AppClassLoader去加载实现类
...
for (DriverInfo aDriver : registeredDrivers) {
    ...
    // skip it.
    // 使用 Thread.currentThread().getContextClassLoader();加载
    // 里面aClass =  Class.forName(driver.getClass().getName(), true, classLoader);
...
private static boolean isDriverAllowed(Driver driver, ClassLoader classLoader) {
    ...
    if (driver != null) {
        ...
        try {
            // 破坏双亲委派机制就在这里
            // Thread.currentThread().getContextClassLoader()类加载器加载实现类并返回实例
