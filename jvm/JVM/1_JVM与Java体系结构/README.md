...
- 追求极客的精神
  - 比如：垃圾回收算法、JIT（及时编译器）、底层原理、AOT (Ahead-Of-Time - 预先编译)
...
这里的编译器和Java虚拟机的Jit编译器是有区别的，这里的编译器是前端编译器，编译代码成class文件的
...
Class文件中包含了Java虚拟机指令集（或者称为字节码、Bytecodes）和符号表，还有一些其他辅助信息。
